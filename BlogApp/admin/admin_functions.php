...
function createAdmin($request_values){
	...
	$user_check_query = "SELECT * FROM users2 WHERE username='$username' 
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO users2 (username, email, role, password, created_at, updated_at) 
...
function editAdmin($admin_id)
{
	...
	$sql = "SELECT * FROM users2 WHERE id=$admin_id LIMIT 1";
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "UPDATE users2 SET username='$username', email='$email', role='$role', password='$password' WHERE id=$admin_id";
		mysqli_query($conn, $query);
...
function deleteAdmin($admin_id) {
	global $conn;
	$sql = "DELETE FROM users2 WHERE id=$admin_id";
...
function getAdminUsers(){
	global $conn, $roles;
	$sql = "SELECT * FROM users2 WHERE role IS NOT NULL";
	$result = mysqli_query($conn, $sql);
